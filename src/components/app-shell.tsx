  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background text-foreground">
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <div
          className={cn(
            "flex shrink-0 items-center gap-3 bg-sidebar px-3 md:hidden",
            marca ? "h-20" : "h-12",
          )}
        >
          <img
            ...
            alt={`Logo ${config.nomeLoja}`}
            className={cn(
              "shrink-0 rounded-full object-cover ring-2 ring-primary",
              marca ? "size-16" : "size-8",
            )}
          />
          <span
            className={cn(
              "min-w-0 flex-1 truncate font-display leading-none tracking-wide text-sidebar-foreground",
              marca ? "text-2xl" : "text-lg",
            )}
          >
            {config.nomeLoja}
          </span>
      ...
      {marca ? (
        <div
          className={cn(
            ...
            recolhido ? "lg:flex lg:w-80 xl:w-96" : "xl:flex xl:w-72",
          )}
        >
          {marca}
        </div>
      ...
      {aside ? (
        <aside
          style={recolhido && corMarca ? { background: corMarca.suave } : undefined}
          className={cn(
            "hidden shrink-0 flex-col transition-all duration-300 ease-out lg:flex",
            recolhido ? "w-12" : "w-80 border-l border-border bg-secondary/40",
          )}
        >
          ...
          >
            {recolhido ? (
              <>
                ...
                <span className="mt-2 [writing-mode:vertical-rl] tracking-widest uppercase">
                  Contas
                </span>
              </>
            ) : (
          ...
          {recolhido ? null : <div className="flex min-h-0 flex-1 flex-col">{aside}</div>}
        </aside>
      ) : null}
