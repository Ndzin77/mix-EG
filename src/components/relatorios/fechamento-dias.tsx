...
export function FechamentoDias({
  ...
  carregando: boolean;
  titulo: string;
  onExportar: (modo: "resumo" | "detalhe", semDinheiro: boolean) => void;
  baixando: boolean;
}) {
    ...
    <section className="overflow-hidden rounded-xl border border-border bg-card">
      <div className="flex flex-wrap items-center gap-3 border-b border-border px-5 py-3">
        ...
        <div ref={caixaMenu} className="no-print relative">
          ...
          {menu ? (
            <div className="modal-in absolute right-0 z-30 mt-2 w-72 overflow-hidden rounded-xl border-2 border-border bg-card shadow-2xl">
              <label className="flex cursor-pointer items-start gap-2.5 border-b border-border bg-secondary/40 px-4 py-3">
                <input
                  type="checkbox"
                  checked={semDinheiro}
                  onChange={(e) => setSemDinheiro(e.target.checked)}
                  className="mt-0.5 size-4 accent-[var(--color-primary)]"
                />
                <span>
                  <span className="block text-sm font-black">Ignorar dinheiro</span>
                  <span className="block text-xs text-muted-foreground">
                    Só PIX e cartão, na entrada e na saída
                  </span>
                </span>
              </label>
              {(
