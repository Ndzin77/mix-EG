...
export function GradeProdutos({
  ...
  return (
    <div className="flex min-h-0 flex-col gap-4">
      {/* Filtro: só o botão, sem barra de separação. */}
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              if (filtroAberto) {
                /* Fechar limpa tudo: some com o cardápio também. */
                setFiltroAberto(false);
                setAba(null);
              } else {
                /* Abrir o filtro já mostra o cardápio inteiro. */
                setFiltroAberto(true);
                if (aba === null) setAba(TODOS);
              }
            }}
          ...
          {visiveis.map((p, i) => {
            ...
            <button
              ...
              className={cn(
                ...
                "bg-background text-left hover:border-primary hover:shadow-xl",
              )}
            >
              <span className="relative block aspect-[4/3] w-full overflow-hidden bg-background">
                {foto ? (
                  <img
                    src={foto}
                    alt={p.nome}
              ...
              <Plus className="absolute right-2.5 top-2.5 size-7 rounded-full bg-background/90 p-1 text-primary opacity-0 shadow transition-opacity group-hover:opacity-100" />
            </button>
            );
          })}
        </div>
