...
function RelatoriosPage() {
  ...
  async function exportar(modo: "resumo" | "detalhe", semDinheiro = false) {
    setBaixando(true);
    const sufixo = semDinheiro ? "-sem-dinheiro" : "";
    try {
      if (modo === "resumo") {
        ...
        baixarCsv(
          `relatorio-resumo${sufixo}-${periodo.de}-a-${periodo.ate}`,
          linhasResumo(linhasDia, semDinheiro),
        );
        return;
      }
      const [mov, linhasDia] = await Promise.all([
        buscarMovimento({ data: { ...periodo, offsetMin } }),
        buscarDias({ data: { ...periodo, offsetMin } }),
      ]);
      if (!mov.length) {
        toast.error("Nenhum movimento nesse período para exportar");
        return;
      ...
      baixarCsv(
        `relatorio-detalhe${sufixo}-${periodo.de}-a-${periodo.ate}`,
        linhasDetalhe(mov, linhasDia, semDinheiro),
      );
