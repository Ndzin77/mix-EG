...
import { CheckCircle2, CreditCard, ShieldAlert, ShieldCheck, Sparkles } from "lucide-react";
...
import { CARENCIA_DIAS, diasAteVencer, type EstadoAssinatura } from "@/lib/assinatura";
...
function Anel({
  ...
  /* O ciclo é mensal: 30 dias como referência do anel. */
  const fracao = emDia ? Math.min(1, Math.max(0.02, faltam / 30)) : 0;
  const raio = 78;
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4">
        <div className="min-w-0">
          <span className="eyebrow text-muted-foreground">
            Plano {estado?.plano ?? "mensal"} · sem fidelidade
          </span>
          <p className="money mt-1 text-4xl font-black leading-none">
            R$ {brl(estado?.valor ?? 39.9)}
            <span className="ml-1 text-sm font-bold text-muted-foreground">/mês</span>
          </p>
        </div>
        <span
          className={cn(
