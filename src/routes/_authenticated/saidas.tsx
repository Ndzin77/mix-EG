...
import { origens, rotuloOrigem, type Origem } from "@/lib/origens";
...
type Lancamento = { description: string; amount: number; category: string; origem: Origem };
...
function SaidasPage() {
  ...
  /* Dinheiro é o caso mais comum no balcão: já vem escolhido. */
  const [origem, setOrigem] = useState<Origem>("cash");
  ...
  const registrar = () => {
    ...
    criar.mutate({ description: desc.trim(), amount: numero, category: cat, origem });
    setDesc("");
    setValor("");
  };
  ...
  return (
    <AppShell>
      <TravaSecao secao="saidas" titulo="Saídas">
      <PageHeader
        title="Saídas"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-5 pb-24 md:pb-5">
        ...
        <section className="rounded-2xl border-l-4 border-danger bg-card p-5 shadow-sm">
          ...
          {/* De onde saiu vem antes da categoria: é a pergunta que muda o
              dinheiro na gaveta, não só o relatório. */}
          <div className="mt-4 flex flex-wrap gap-2">
            <span className="flex items-center gap-1 self-center text-xs font-bold text-muted-foreground">
              De onde saiu
              <Dica texto="Só saída em dinheiro tira cédula da gaveta. PIX e cartão saem da conta do banco e não entram na conferência da gaveta." />
            </span>
            {origens.map((o) => (
              <button
                key={o.chave}
                onClick={() => setOrigem(o.chave)}
                aria-pressed={origem === o.chave}
                className={cn(
                  "press flex items-center gap-1.5 rounded-xl border px-4 py-2.5 text-sm font-bold transition-colors",
                  origem === o.chave
