...
export const obterLoja = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      .from("store_settings")
      .select(
        "tenant_id, store_name, phone, address, receipt_footer, logo_url, salao_ativo, termo_mesa, qtd_mesas, destinos, meta_diaria, alerta_min, atraso_min, caixa_privado, categorias_saida, categorias_produto, cronometro_ativo, recibo_config, bloqueios, preparo_ativo, marca",
      )
      .maybeSingle();
    if (error) throw new Error(error.message);
    return data;
