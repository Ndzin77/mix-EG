...
import type { Secao, Trava } from "@/lib/travas";
...
export type Config = {
  ...
  recibo: ReciboConfig;
  /** cadeado opcional por seção: só o resumo embaralhado da senha */
  bloqueios: Partial<Record<Secao, Trava>>;
  /** fila de preparo (experimental): tela do ajudante montando os pedidos */
  preparoAtivo: boolean;
  /** como a logo aparece na tela de vendas */
  marca: MarcaConfig;
};

/** Presença da marca no PDV: totem lateral, marca d'água e modo vitrine. */
export type MarcaConfig = {
  /** coluna fixa com a logo enorme durante a venda */
  totem: boolean;
  /** largura da logo no totem, em % da coluna (50–100) */
  tamanho: number;
  /** opacidade da logo do totem, em % (0–100; 100 = sólida) */
  opacidadeTotem: number;
  /** logo translúcida atrás da grade de produtos */
  marcaDagua: boolean;
  /** presença da marca d'água, em % (0–100) */
  intensidade: number;
  /** tela cheia com a logo depois de um tempo parado */
  vitrine: boolean;
  /** segundos de inatividade até a vitrine entrar */
  vitrineSegundos: number;
  /** o fundo do balcão pega a cor dominante da logo, seja ela qual for */
  combinar: boolean;
  /** a gaveta de contas passa por cima do balcão em vez de empurrá-lo */
  gavetaCobre: boolean;
};
...
/* Antes os controles eram três degraus; agora são contínuos. Quem já salvou
   "gigante" ou "forte" continua vendo exatamente o mesmo balcão. */
const TAMANHO_ANTIGO: Record<string, number> = { media: 70, grande: 85, gigante: 98 };
const INTENSIDADE_ANTIGA: Record<string, number> = { sutil: 25, media: 45, forte: 80 };

const entre = (v: number, min: number, max: number) => Math.min(max, Math.max(min, v));

/** Lê a preferência de marca venha ela do banco, do navegador ou de outra era. */
export function normalizarMarca(bruto: unknown): MarcaConfig {
  const b = (bruto && typeof bruto === "object" ? bruto : {}) as Record<string, unknown>;
  const numero = (v: unknown, antigos: Record<string, number>, padrao: number) => {
    if (typeof v === "number" && Number.isFinite(v)) return v;
    if (typeof v === "string" && antigos[v] != null) return antigos[v]!;
    return padrao;
  };
  return {
    ...marcaPadrao,
    ...(b as Partial<MarcaConfig>),
    tamanho: entre(numero(b.tamanho, TAMANHO_ANTIGO, marcaPadrao.tamanho), 50, 100),
    intensidade: entre(numero(b.intensidade, INTENSIDADE_ANTIGA, marcaPadrao.intensidade), 0, 100),
    opacidadeTotem: entre(
      typeof b.opacidadeTotem === "number" ? b.opacidadeTotem : marcaPadrao.opacidadeTotem,
      0,
      100,
    ),
    gavetaCobre: b.gavetaCobre === true,
    vitrineSegundos: Math.max(
      15,
      typeof b.vitrineSegundos === "number" ? b.vitrineSegundos : marcaPadrao.vitrineSegundos,
    ),
  };
}
...
export function configDaLinha(linha: LinhaLoja | null | undefined): Partial<Config> {
  ...
  if (linha.bloqueios && typeof linha.bloqueios === "object") {
    p.bloqueios = linha.bloqueios as Partial<Record<Secao, Trava>>;
  }
  if (linha.preparo_ativo != null) p.preparoAtivo = linha.preparo_ativo;
  if (linha.marca && typeof linha.marca === "object") {
    p.marca = normalizarMarca(linha.marca);
  }
...
export function linhaDaConfig(patch: Partial<Config>): Record<string, unknown> {
  ...
  if (patch.bloqueios !== undefined) l.bloqueios = patch.bloqueios;
  if (patch.preparoAtivo !== undefined) l.preparo_ativo = patch.preparoAtivo;
  if (patch.marca !== undefined) l.marca = patch.marca;
  return l;
}
...
function carregar(): Config {
  ...
  try {
    ...
    if (bruto) {
      ...
      atual = { ...configPadrao, ...salvo, marca: normalizarMarca(salvo.marca) };
    }
