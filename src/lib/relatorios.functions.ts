...
export const fechamentoDiario = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [pedidosRes, saidasRes] = await Promise.all([
      ...
      supabase
        .from("expenses")
        .select("description, category, amount, occurred_at, origem")
    ...
    type Dia = {
      ...
      saidas: { descricao: string; categoria: string; valor: number; origem: string }[];
      saiuDinheiro: number;
      pedidos: string[];
    };
    ...
    for (const s of saidasRes.data ?? []) {
      ...
      const v = Number(s.amount ?? 0);
      const origem = (s as { origem?: string | null }).origem ?? "cash";
      alvo.saiu += v;
      if (origem === "cash") alvo.saiuDinheiro += v;
      alvo.saidas.push({
    ...
    return [...dias.values()]
      ...
      .map((d) => ({
        ...
        saiu: arred(d.saiu),
        saiuDinheiro: arred(d.saiuDinheiro),
...
export const movimentoDetalhado = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    /* Quanto de cada pedido entrou em dinheiro: a planilha "só PIX e cartão"
       precisa saber se a venda foi toda na cédula. */
    const dinheiroPorPedido = new Map<string, number>();
    for (const o of pedidos) {
      const partes = (partesRes.data ?? []).filter((p) => p.order_id === o.id) as never;
      const f = somarFormas([o] as never, partes);
      dinheiroPorPedido.set(o.id, arred(f.cash ?? 0));
    }

    const info = new Map<
      string,
      { conta: string; quando: string; desconto: number; pagamento: string; dinheiro: number }
    >();
    for (const o of pedidos) {
      const detalhe = pagosPorPedido.get(o.id);
      ...
      info.set(o.id, {
        ...
        desconto: arred(Number(o.discount ?? 0)),
        pagamento,
        dinheiro: dinheiroPorPedido.get(o.id) ?? 0,
      });
    }
    ...
    const vendas = (itensRes.data ?? []).map((i) => {
      ...
      return {
        ...
        /** venda recebida inteiramente em cédula: sai da planilha sem dinheiro */
        soDinheiro: (dados?.dinheiro ?? 0) > 0 && !(dados?.pagamento ?? "").includes("+"),
      };
    });
    ...
    const saidas = (saidasRes.data ?? []).map((s) => ({
      ...
      desconto: 0,
      pagamento: rotuloForma[(s as { origem?: string | null }).origem ?? "cash"] ?? "Dinheiro",
      soDinheiro: (((s as { origem?: string | null }).origem ?? "cash") === "cash"),
    }));
